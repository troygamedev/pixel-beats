...
public class GameControllerScript : MonoBehaviour
{
    ...
    void OnNoteClicked(Note note) {
        ...
        if(multiplierTierIndex != multiplierTiers.Length - 1) {
            if (streak > multiplierTiers[multiplierTierIndex + 1].streaksNeeded) {
                multiplierTierIndex++;
                UpdateNewMultiplerTier(multiplierTierIndex);
            }
        }
    ...
    void UpdateNewMultiplerTier(int index) {
        if(index == 0) {
            multiplierText.gameObject.SetActive(false);
        } else {
            multiplierText.gameObject.SetActive(true);
        }
        scoreMultiplier = multiplierTiers[index].multiplier;
        multiplierText.text = "x" + multiplierTiers[index].multiplier;
        multiplierText.color = multiplierTiers[index].streakColor;
    }
    ...
    IEnumerator GameOver() {
        ...
        if (!isPreviewMode) {
            ...
            PlayerPrefs.SetFloat("HighestAccuracy", (1f * notesHit / notesTotal));
